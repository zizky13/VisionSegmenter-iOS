...
struct ContentView: View {
    @State private var inputImage: UIImage? = UIImage(named: "heavy_test")
        ...
        var body: some View {
            VStack(spacing: 20) {
                ...
                if let inputImage = inputImage {
                    ...
                    Text("No input image loaded.")
                }

                // ... inside the body of ContentView ...
                            Button("Perform Segmentation") {
                                // Make sure we have an image to work with
                                guard let imageToProcess = inputImage else { return }
                                performSegmentation(for: imageToProcess)
                            }
                            // ... rest of the body ...
                .padding()
                ...
                if let segmentedImage = segmentedImage {
                    ...
                    Rectangle() // Placeholder for the output
                        .fill(Color.gray.opacity(0.2))
                        .frame(height: 250)
                        .cornerRadius(10)
                        .overlay(Text("Output will appear here").foregroundColor(.gray))
                }
    ...
    func performSegmentation(for image: UIImage) {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            do {
                ...
                if let compositeMask = createCompositeMask(from: observations, for: cgImage) {
                    applyMask(mask: compositeMask, to: image)
                }
    ...
    // NEW HELPER FUNCTION to handle multiple instances
    func createCompositeMask(from observations: [VNInstanceMaskObservation], for image: CGImage) -> CVPixelBuffer? {
        ...
        var compositeMaskImage = CIImage(color: .black).cropped(to: CGRect(x: 0, y: 0, width: imageWidth, height: imageHeight))
        ...
        CVPixelBufferCreate(kCFAllocatorDefault, imageWidth, imageHeight, kCVPixelFormatType_OneComponent8, nil, &pixelBuffer)
    ...
    func applyMask(mask: CVPixelBuffer, to originalImage: UIImage) {
        ...
        filter.backgroundImage = CIImage(color: .black).cropped(to: originalCIImage.extent) // Black background
